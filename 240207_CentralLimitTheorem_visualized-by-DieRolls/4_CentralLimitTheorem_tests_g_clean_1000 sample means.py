...
TEST:
- We randomly roll a dice 5 times per set.
- We calculate the mean of this batch.
- We then run this batch run 100 times.
- Then we compute the sample distribution on 
10:53 07/01/2024
"""
# ------------------------------------------------------------------- STAT CALLS
...
# ---------------------------------------------------------------- VIZ UTILITIES
# command-line table enumarator viz
...
def plotDualLstAsTable_str(numLst1,numLst2):
    ...
    for i,j in enumerate(numLst1):
        numStr = str(i+1)
        if len(numStr) == 1:
            numStr = "0"+numStr   # amendment for prettyPrinting!
        strLocal = "| " + numStr + " | " + str(j) +  " | " + str(numLst2[i]) + " |"
        strMega = strMega + "\n" + strLocal
    #print hSepStr
    return strMega
# -------------------------------------------------------------------- UTILITIES
# This is the standard dice face list
#print(range(1,7,1))
...
# PERFORM THE DICE ROLLS IN X BATCHES (Currently X = 100)
# 100 sets of 5 die rolls.
loopedMeanLst = meanXRolls(1000,5)
####plotAsTable(loopedMeanLst)
